    ...
    <div className="relative min-h-dvh bg-white text-slate-900">
      ...
      {!hideNav && (
        <nav className="fixed bottom-4 left-0 right-0 z-[100] px-4 pb-[env(safe-area-inset-bottom)]">
          <div className="mx-auto max-w-xl rounded-[32px] border border-white/70 bg-white/75 backdrop-blur-2xl shadow-[0_20px_60px_rgba(15,23,42,0.18)] ring-1 ring-black/5">
              ...
              {links.map(({ to, label, icon: Icon }) => {
                ...
                return (
                  <Link
                    key={to}
                    to={to}
                    className="group flex flex-col items-center gap-1.5 text-[11px] font-semibold text-slate-500 transition-all duration-150"
                  >
                    <span
                      className={`grid h-11 w-11 place-items-center rounded-2xl transition-all duration-200 ${
                        activeTab
                          ? "bg-neutral-900 text-white shadow-[0_12px_32px_rgba(15,23,42,0.28)] ring-1 ring-white/40 -translate-y-0.5"
                          : "bg-white/80 text-slate-600 ring-1 ring-black/5 group-hover:text-slate-900 group-hover:shadow-md group-hover:-translate-y-0.5"
                      }`}
                    >
